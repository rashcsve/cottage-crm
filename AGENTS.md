...
- **Demo mode** (`NEXT_PUBLIC_DEMO_MODE=1`): on the isolated demo deployment only, every route an unauthenticated visitor can reach except the home page auto-redirects through `app/api/demo/session/route.ts`, which signs them in as a real, pre-seeded Supabase user (`DEMO_USER_EMAIL` / `DEMO_USER_PASSWORD`, server-only env vars) via `supabase.auth.signInWithPassword()` — this covers `/login`, `/signup`, and any dashboard route hit directly (`getCurrentProfile()`), with no exception for a visitor who just signed out. The home page alone shows the normal hero content with "Try the demo" CTAs pointing at that same route (see `redirectIfAuthenticated({ skipDemoAutoLogin: true })` in `app/[locale]/page.tsx`) — signing out redirects there, which is why it doesn't loop back into an immediate re-login. This is a real Supabase session — RLS is untouched, `requireUser()`/`requireAdmin()` behave exactly as in production. The route is rate-limited and rejects non-navigation requests (see `lib/demo/rate-limit.ts`). See `lib/demo/`. Do not let this flag default to on, and do not use it to bypass `requireUser()`/`requireAdmin()`/RLS directly — it only ever produces a normal authenticated session for a real seeded account.
